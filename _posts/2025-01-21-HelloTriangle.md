...
## 요약
> **요약**: 그래픽스 파이프라인에서 삼각형을 그리기 위해 필요한 3가지 핵심 요소(VBO/EBO, 셰이더 프로그램, 속성 연결)와 전체적인 렌더링 과정을 정리한다.
...
*   **버텍스 셰이더(Vertex Shader)**는 각 정점의 변환을 처리한다. 예를 들어, 모델을 화면에 그리기 위한 좌표 변환(모델, 뷰, 프로젝션 변환)을 수행한다.
*   **프래그먼트 셰이더(Fragment Shader)**는 픽셀 색상 계산을 담당한다. 각 픽셀에 대한 색상, 텍스처 등을 계산한다.
...
## 정리
그래픽 파이프라인에서 삼각형을 그리기 위해서는 **GPU 메모리에 데이터를 저장**하고, **셰이더 프로그램을 활성화**하며, **버텍스 속성을 셰이더와 연결**하는 과정이 필요하다. 이 세 가지 조건을 통해 OpenGL이 데이터를 처리하고 화면에 객체를 그릴 수 있게 된다.
