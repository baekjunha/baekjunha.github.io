# crawling section4


## **전체 개요:**

**웹 크롤링(Web Crawling)**이란,

웹에 있는 정보를 자동으로 가져와서 내가 필요한 데이터만 추출하는 작업이다.

파이썬에서는 **requests**와 **BeautifulSoup**이라는 라이브러리를 통해 간단하게 구현할 수 있다.

---

## **1. 라이브러리 임포트**

import requests  
from bs4 import BeautifulSoup

### **설명:**

* requests: 특정 웹 페이지에 접속하고, 그 페이지의 HTML을 파이썬 코드로 가져오기 위해 사용한다.  
* BeautifulSoup: HTML 문서를 구조적으로 분석해 내가 원하는 정보를 쉽게 추출할 수 있도록 도와주는 라이브러리이다.

이 두 라이브러리는 크롤링을 할 때 기본 도구라고 생각하면 된다.  
---

## **2. 웹 페이지 가져오기**

res = requests.get('http://v.media.daum.net/v/20170615203441266')

### **설명:**

* requests.get(URL)은 해당 URL의 HTML 데이터를 요청(request)하는 함수이다.  
* 반환된 res 객체에는 다양한 정보가 담겨 있으며, **웹사이트의 전체 HTML 코드**는 res.content로 확인할 수 있다.

### **res.content**

### **의 특징:**

* HTML 소스 전체가 **바이트(byte) 형태**로 들어있다.  
* 실제 페이지 소스 보기(Ctrl + U) 했을 때와 비슷한 HTML 코드가 출력된다.

---

## **잠깐! 웹 기본 구조 이해하기**

HTML 웹 문서의 구조는 대략 아래와 같이 되어 있다.

<html>  
  <head>  
    <title>웹페이지 제목</title>  
  </head>  
  <body>  
    <p class="normal">본문 첫 번째 문장</p>  
    <p class="special">두 번째 문장 <b>굵게 표시됨</b></p>  
  </body>  
</html>

### **주요 태그 설명:**

* <html>: 문서의 루트 요소  
* <head>: 문서의 설정(제목, 스타일 등)이 들어가는 부분  
* <title>: 브라우저 탭에 표시될 제목  
* <body>: 본문 내용이 들어가는 영역  
* <p>: 단락(문단), <b>: bold(굵은 글씨)

---

## **3. 웹 페이지 파싱(Parsing) 하기**

soup = BeautifulSoup(res.content, 'html.parser')

### **설명:**

* HTML 문서를 **'html.parser' 파서(parser)** 를 사용해 BeautifulSoup 객체로 변환한다.  
* 이 과정은 **단순 텍스트인 HTML을 구조화된 트리 구조**로 바꾸어 주며, 이후에 원하는 정보를 쉽게 찾을 수 있다.

soup 객체는 HTML 트리 전체를 메모리에 가지고 있으며, 이 안에서 find, find_all, select 같은 함수로 원하는 태그를 탐색할 수 있다.  
---

## **4. 원하는 데이터 추출하기**

mydata = soup.find('title')  
print(mydata.get_text())

### **설명:**

* soup.find('태그명'): HTML 안에서 첫 번째로 해당 태그를 찾아 반환  
  * 여기서는 <title> 태그를 찾는다.  
* .get_text(): 해당 태그 내부의 **순수 텍스트**만 추출 (HTML 태그 제거됨)

### **예시 결과:**

<title>문재인 "한·미, 북핵 문제 해결 위해 긴밀히 협의"</title>

get_text() 실행 결과:

문재인 "한·미, 북핵 문제 해결 위해 긴밀히 협의"  
---

## **5. 추출한 데이터 활용하기**

print(mydata.get_text())

* 이렇게 추출한 데이터를 출력할 수도 있고,  
* 파일에 저장하거나,  
* 다른 분석 작업에 활용할 수도 있다.

예:

# 파일로 저장  
with open('headline.txt', 'w', encoding='utf-8') as f:  
    f.write(mydata.get_text())  
---

## **패턴 총정리: 재사용 가능한 기본 틀**

import requests  
from bs4 import BeautifulSoup  
  
res = requests.get('URL을 여기에 입력하세요')  
soup = BeautifulSoup(res.content, 'html.parser')  
data = soup.find('찾고 싶은 태그')  
print(data.get_text())

### **바꿔야 할 두 부분:**

1. requests.get() 안의 **URL**  
2. soup.find() 안의 **HTML 태그명**  
   (또는 class, id 등을 활용한 조건 검색 가능)

---

## **추가 팁: 태그 외에도 속성으로 검색 가능**

예를 들어, 특정 class나 id를 가진 요소를 찾고 싶다면?

soup.find('p', class_='special')  
soup.find(id='headline')

또는 여러 개를 찾고 싶다면:

soup.find_all('p')  # 모든 <p> 태그 반환 (리스트 형태)  
---

## **마무리**

이 문서는 웹 크롤링의 **가장 기본적인 흐름과 패턴**을 익히는 데 초점이 맞춰져 있다.

한 번 따라해보면서 아래 5단계 흐름을 체득하면, 다양한 사이트에 응용이 가능하다:

1. 라이브러리 임포트  
2. HTML 가져오기 (requests)  
3. HTML 파싱 (BeautifulSoup)  
4. 원하는 요소 찾기 (find, get_text)  
5. 데이터 활용하기 (출력, 저장, 가공 등)

---

## **1. 웹페이지의 구성요소: HTML, CSS, JavaScript**

* 웹 페이지는 크게 세 가지 언어로 구성된다:  
  * **HTML**: 웹페이지의 **구조(structure)** 를 담당 (뼈대)  
  * **CSS**: 웹페이지의 **디자인(스타일)** 을 담당 (색상, 글꼴, 정렬 등)  
  * **JavaScript**: 웹페이지의 **동작/기능** 을 담당 (클릭, 애니메이션 등)

---

## **2. HTML 이해하기 (웹 크롤링의 핵심)**

### **HTML이란?**

* **HyperText Markup Language**의 약자  
* 웹 페이지의 정보를 표시하는 마크업 언어  
* <태그>를 사용해 구조를 정의한다

---

### **HTML 기본 구조 예시:**

<html>  
  <head>  
    <title>웹페이지 제목</title>  
  </head>  
  <body>  
    <h1>큰 제목</h1>  
    <p>본문 내용입니다</p>  
  </body>  
</html>

#### **구성 요소 설명:**

* <html>: HTML 문서의 전체를 감싸는 태그  
* <head>: 문서 정보(제목, 문자 인코딩 등)  
* <title>: 브라우저 탭에 표시되는 제목  
* <body>: **화면에 표시되는 실제 내용** 

---

### **태그(tag)의 형식**

<b>Hello HTML</b>

* 시작 태그: <b>  
* 종료 태그: </b>  
* 태그 내부에 쓰이는 문자열: **텍스트 콘텐츠**  
* 태그는 **중첩**이 가능하다.

<p>이 문장 안에 <b>굵은 글씨</b>가 있어요.</p>  
---

### **HTML 태그의 속성(attribute)**

속성은 태그 안에서 다양한 부가정보를 제공한다.

예:

<img src="dimg.png" width="100" height="100">

* <img>: 이미지 삽입 태그  
* src: 이미지 파일 경로  
* width: 가로 길이  
* height: 세로 길이

태그마다 가질 수 있는 속성이 다르며, **크롤링 시 속성값(class, id 등)을 활용해 데이터를 찾는다**.

---

### **중요 속성 – id, class**

이 두 속성은 **특정 요소를 지정하거나 스타일을 적용할 때, 또는 크롤링 시 데이터를 정확히 찾는 데 매우 중요**하다.

<h1 id="title">웹 제목</h1>  
<p class="content">본문입니다</p>

* id="title": 고유한 식별자 (하나의 HTML 문서 내에 하나만 존재)  
* class="content": 같은 스타일이나 그룹을 묶을 때 사용 (여러 요소에 반복 사용 가능)

크롤링할 때 BeautifulSoup에서 find(id='title'), find(class_='content') 와 같이 활용된다.  
---

## **3. 문자 인코딩 – <meta charset="utf-8">**

HTML 파일을 브라우저에서 열었을 때 **글자가 깨지는 현상**을 방지하기 위해 사용한다.

<head>  
  <meta charset="utf-8">  
</head>  
     
* UTF-8은 전 세계 언어를 표현할 수 있는 문자 인코딩 방식으로 **한글 크롤링 시 필수**이다.

---

## **4. HTML의 다양한 태그 예시**

| 태그 | 설명 |
| ----- | ----- |
| <h1>~<h6> | 제목 (숫자가 작을수록 글자가 큼) |
| <p> | 문단 |
| <a href="URL"> | 하이퍼링크 |
| <img src="이미지경로"> | 이미지 삽입 |
| <table>, <tr>, <td> | 표(Table) 작성 |
| <div>, <span> | 구역 설정/구조화 요소 |

이런 태그를 잘 알고 있어야 **크롤링할 때 내가 원하는 데이터가 어디에 들어있는지 파악**할 수 있다.

올려주신 코드는 아주 기본적인 HTML 문서로, 웹 페이지에서 **텍스트, 이미지, 표(table)** 를 표현하는 구조를 가지고 있다. 아래에 각 줄과 구성요소를 **차근차근 친절하게 설명**드린다.

---

## **전체 코드 분석 - htmlbasic.html**

**<!DOCTYPE html>**

**<html>**

* <!DOCTYPE html>: 이 문서는 HTML5 문서임을 브라우저에 알려주는 선언이다.  
* <html>: 전체 HTML 문서의 시작이다. 이 태그 안에 웹 페이지의 모든 내용이 들어간다.

---

**<head>**

	**<title>HTML TEST</title>**

	**<meta charset="utf-8">**

	**<link rel="stylesheet" type="text/css" href="css/style.css">**

**</head>**

### **head 태그 영역**

* <head>는 사용자에게 보이지 않지만, 문서의 설정 정보(meta-data)를 포함한다.

#### **각 요소 설명:**

| 태그 | 설명 |
| ----- | ----- |
| <title> | 브라우저 탭에 표시될 제목이다. 여기서는 **HTML TEST** 라고 나온다. |
| <meta charset="utf-8"> | 한글이나 다양한 문자가 깨지지 않게 **UTF-8 인코딩**을 설정한다. |
| <link rel="stylesheet" href="css/style.css"> | 외부에 있는 CSS 파일을 연결하여 이 HTML의 스타일을 지정한다. 예를 들어, 특정 글자에 색을 입히거나 크기를 조절할 수 있다. |

---

**<body>**	

	**<b>안녕</b><br>**

	**<img src="python-logo.png">**

### **body 태그 영역**

* <body>는 사용자에게 보이는 실제 콘텐츠가 들어가는 부분이다.

#### **각 요소 설명:**

| 태그 | 기능 |
| ----- | ----- |
| <b>안녕</b> | 글자를 **굵게** 표시한다. 화면에는 **안녕** 이 진하게 보인다. |
| <br> | 줄 바꿈을 의미한다. (<p> 대신 간단히 줄 바꿀 때 사용) |
| <img src="python-logo.png"> | 이미지를 삽입한다. 이때 "python-logo.png"라는 이미지 파일이 프로젝트 폴더에 있어야 한다. |

---

**<table border="1" width="500">**

	**<thead>**

		**<tr>**

			**<th>title1</th>**

			**<th>title2</th>**

		**</tr>**

	**</thead>**

### **테이블의 헤더 영역**

* <table>: 표(table)를 만드는 태그이다.  
  * border="1": 테두리를 1px 두께로 표시한다.  
  * width="500": 표 전체 너비를 500px로 지정한다.  
* thead  
* <tr>: 한 줄(row)을 의미한다.  
* <th>: **테이블의 제목 셀(header)** 을 의미하며, 굵고 가운데 정렬된다.

예:

| title1 | title2 |

---

	**<tbody>**

		**<tr>**

			**<td class="highlight">안녕1</td>**

			**<td>안녕2</td>**

		**</tr>**

	**</tbody>**

### **테이블의 본문 영역**

* <tbody>: 실제 데이터 내용이 들어가는 영역이다.  
* <td>: 테이블의 한 칸(데이터 셀)을 의미한다.  
* class="highlight": CSS에서 이 셀만 따로 스타일을 적용할 수 있도록 **클래스 지정**을 한 것이다.

이 부분은 표에서 다음과 같이 보일 것이다:

| 안녕1 | 안녕2 |

---

## **전체 구조 요약**

| 구역 | 설명 |
| ----- | ----- |
| <head> | 문서 정보 (제목, 문자 인코딩, CSS 연결 등) |
| <body> | 사용자 화면에 표시될 실제 콘텐츠 |
| <b> | 텍스트를 굵게 표시 |
| <br> | 줄 바꿈 |
| <img> | 이미지 삽입 |
| <table> | 표 생성 (제목 + 본문) |
| <th> | 표의 제목 셀 (헤더) |
| <td> | 표의 데이터 셀 (본문) |
| class="..." | CSS로 스타일을 지정하기 위한 이름 지정 |

---

## **예시 CSS - style.css에 들어갈 수 있는 내용**

## **전체 코드**

**td {**

	**font-size: 2em;**

	**font-family: Gulim;**

	**text-align: center;**

**}**

**.highlight {**

	**font-size: 3em;**

	**text-align: right;**

	**color: blue;**

**}**

---

## **1. td 선택자**

**td {**

	**font-size: 2em;**

	**font-family: Gulim;**

	**text-align: center;**

**}**

### **이 스타일은 모든 <td> 태그에 적용된다.(즉, 테이블의 일반적인 데이터 셀에 적용)**

| 속성 | 의미 |
| ----- | ----- |
| **font-size: 2em;** | **글자 크기를 기본보다 2배 크게 표시한다.1em은 브라우저 기본 글자 크기 (보통 16px) 기준** |
| **font-family: Gulim;** | **글꼴을 굴림체로 지정한다 (한글 시스템 글꼴)** |
| **text-align: center;** | **텍스트를 가운데 정렬한다** |

**적용 예시:**

**<td>표 내용</td> → 글자가 굴림체, 크기 2배, 가운데 정렬**

---

## **2. '.highlight' 클래스 선택자**

**.highlight {**

	**font-size: 3em;**

	**text-align: right;**

	**color: blue;**

**}**

### **이 스타일은 class="highlight"속성을 가진 요소에만 적용된다.**

**예: <td class="highlight">강조된 셀</td>**

| 속성 | 의미 |
| ----- | ----- |
| **font-size: 3em;** | **글자를 기본보다 3배 크게 표시** |
| **text-align: right;** | **텍스트를 오른쪽 정렬** |
| **color: blue;** | **글자 색을 파란색으로 설정** |

**HTML에서 사용 예시:**

**<td class="highlight">안녕1</td>**

**→ 이 셀은 3배 크기의 파란 글씨로, 오른쪽 정렬되어 나타난다.**

---

## **td vs .highlight 우선순위**

* **td는 모든 <td> 태그에 적용되는 기본 스타일**  
* **.highlight는 특정한 class="highlight"가 지정된 셀에만 적용되는 우선 스타일**

**같은 요소에 두 스타일이 동시에 적용되면, .highlight가 더 우선한다.**

**예:**

**<td class="highlight">적용 결과</td>**

* **글꼴: Gulim (기본 td에서 적용)**  
* **글자 크기: 3em (.highlight가 덮어씀)**  
* **정렬: right (.highlight가 덮어씀)**  
* **색상: blue (.highlight에만 있음)**

---

## **정리**

| 선택자 | 적용 대상 | 스타일 |
| ----- | ----- | ----- |
| **td** | **모든 <td> 태그** | **글자 크기 2배, 굴림체, 가운데 정렬** |
| **.highlight** | **class="highlight"가 있는 태그** | **글자 크기 3배, 파란색, 오른쪽 정렬** |

---

## **추가 팁**

**.highlight 클래스는 다른 태그에도 적용할 수 있다:**

**<p class="highlight">강조된 문장</p>**

**여러 클래스를 같이 쓸 수도 있다:**

**<td class="highlight special">문장</td>**

**CSS는 코드 재사용과 유지보수에 매우 유리하다.**  
**크롤링할 때는 이런 class 정보를 이용해 HTML 구조에서 필요한 데이터를 뽑아낼 수 있다.**

---

## **5. CSS 이해하기 (스타일 정보)**

### **CSS란?**

* **Cascading Style Sheets**  
* HTML 태그를 꾸미는 언어 (디자인 전담)  
* 예: 글자 색, 글자 크기, 정렬 방식 등 지정

---

### **CSS 적용 방식 3가지**

| 방식 | 설명 | 예시 |
| ----- | ----- | ----- |
| 인라인 스타일 | 태그에 직접 작성 | <p style="color:red;">문장</p> |
| 내부 스타일 | HTML 안의 <style> 태그 | <style> p { color:red; } </style> |
| 외부 스타일 | 별도 CSS 파일로 연결 | <link rel="stylesheet" href="style.css"> |

---

### **자주 쓰이는 CSS 속성**

| 속성 | 설명 | 값 예시 |
| ----- | ----- | ----- |
| color | 글자 색 | red, #0277BD |
| font-size | 글자 크기 | 16px, 2em |
| font-family | 글꼴 | Gulim, Arial |
| text-align | 텍스트 정렬 | left, center, right |

---

## **크롤링을 위한 핵심 요약**

| 요소 | 설명 | 크롤링에서의 역할 |
| ----- | ----- | ----- |
| <태그> | 웹페이지 구성 단위 | 원하는 데이터가 들어있는 위치를 파악 |
| id, class | 태그의 속성 | 특정 데이터를 정확하게 찾는 기준 |
| <meta charset="utf-8"> | 문자 인코딩 | 한글 깨짐 방지 |
| CSS 스타일 | 시각적 표현 담당 | 직접적 데이터는 아니지만, 위치나 의미를 유추할 때 도움 |

---

## **마무리**

웹 크롤링을 제대로 하려면:

* **HTML 구조를 잘 이해하고 있어야 하며**  
* **id, class, <div>, <p> 등이 어떻게 쓰이는지** 익숙해져야 한다.  
* **크롬 개발자 도구(F12)** 를 통해 웹사이트 구조를 살펴보는 습관도 중요하다!

---

